 ...
 *@args class head and course name and grade
 *@return 1 or 0 based on success
 * ******************/
int required::remove_class(gen_class* &a_class, char* course_name, float grade_check)
{
    if(!a_class) return 0;
    if(!course_name) return 0;
    if((a_class->check_string(course_name, a_class->get_course()) == 1) && (floor(grade_check) >= floor(a_class->get_grade())))
    {
    ...
    return remove_class((gen_class* &)a_class->get_next(), course_name, grade_check);
}
...
/********************
 *@author harsukh singh CS 202 Assignment 2
 *@function removes class based on course name and grade
 *@args class head and course name
 *@return 1 or 0 based on success
 * ******************/
int required::remove_class(char* course_name, float grade_check)
{
    return remove_class(a_class, course_name, grade_check);
}
...
bool required::is_empty()
{
    if(!a_class) 
    {
      cout<<"Requirements completed"<<endl;
      return true;
    }
    return false;
}
